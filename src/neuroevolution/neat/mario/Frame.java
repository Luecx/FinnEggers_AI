package neuroevolution.neat.mario;

import neuroevolution.neat.visual.NEATFrame;
...
public class Frame extends JFrame implements KeyListener {
    ...
    public static void main(String[] args){
        ...
        int x = 0;
        int[] ranges = new int[]{2,3,4};
        while(x < 120){
            x += ranges[(int)(Math.random() * ranges.length)];
            w.placeBlock(x, 2 - (int) (Math.random() * 2));
        }
    ...
    private class Updater extends Thread{
        ...
        public void run(){
            while(!this.isInterrupted()){
                try{
                    System.out.println(update_timer);
                    frame.revalidate();
